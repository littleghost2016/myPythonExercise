...
class debugger():
    ...
    def load(self, path_to_exe):
        ...
        startupinfo = STARTIPINFO()
        process_information = PROCESS_INFORMATION()
        # The following two options allow the started process to be shown as a separate window.
        # This also illustrates how different settings in the STARTUPINFO struct can affect the debugger.
        startupinfo.dwFlags = 0x1
        startupinfo.wShowWindow = 0x0
        # We then initialize the cb variable in the STARTUPINFO struct
        # which is just the size fo the struct itself
